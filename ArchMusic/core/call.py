...
from pytgcalls.types import MediaStream, Update, StreamAudioEnded, ChatUpdate, GroupCallParticipant
    ...
    async def decorators(self): 
        async def kicked_handler(client, update: Update):
            if update.status == ChatUpdate.Status.KICKED:
                await self.handle_kicked_event(client, update)

        async def participants_change_handler(client, update: Update):
            if isinstance(update, GroupCallParticipant):
                await self.handle_participants_change(client, update)

        async def stream_end_handler(client, update: Update):
            if isinstance(update, StreamAudioEnded):
                await self.handle_stream_end(client, update)

        async def handle_update(client, update: Update):
            await kicked_handler(client, update)
            await participants_change_handler(client, update)
